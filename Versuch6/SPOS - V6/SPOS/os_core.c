...
void os_init(void) {
	...
	if (BOTTOM_OF_HEAP < (uint16_t)&__heap_start){
		os_error("Heap too low");
	}
	
	
	//if(BOTTOM_OF_HEAP > TOP_OF_HEAP){
	//	os_error("Heap too high");
	//}
...
void os_errorPStr(const char *str) {
	// Cause error with os_error("Abc");
	// TODO: Deactive Interrupts?
	uint8_t orig_gieb = SREG & 0b10000000;
	SREG &= 0b01111111;
	lcd_clear();
	lcd_writeErrorProgString(str);
	while (1)
	{
		// Enter + Esc to Ack error
